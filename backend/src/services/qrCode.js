...
function generateQRCodeData(params) {
  ...
  // Format amount (RSD + integer amount)
  // According to spec, amount should be without decimals or with comma for decimals
  const formattedAmount = `RSD${Math.round(amount)}`;
  ...
  return {
    ...
    S: 'Mesecno odrzavanje zgrade',  // Payment purpose (Cyrillic in actual use)
    RO: formattedReference  // Reference number
    // P (Payer) intentionally omitted as per NBS recommendations
  };
}
 ...
 * @returns {Promise<Buffer>} QR code image as buffer
 */
async function generatePaymentQRCode(apartment, building, month, year, size = DEFAULT_QR_SIZE) {
  ...
  const qrData = generateQRCodeData({
    ...
    recipientName: 'Stambena zajednica',  // Fixed Cyrillic text
 ...
 */
async function generatePaymentQRCodeBase64(apartment, building, month, year, size = DEFAULT_QR_SIZE) {
